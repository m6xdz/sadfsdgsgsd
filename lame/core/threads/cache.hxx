    ...
    private:
        ...
        void beatmap_loop( ) {
            ...
            while ( m_running ) {
                ...
                const bool lazer_memory_ready =
                    game_snap.client == osu::client_kind_t::lazer && in_play &&
                    ( game_snap.gameplay_beatmap != 0 || game_snap.drawable_ruleset != 0 );
                const bool can_try_map = has_map || lazer_memory_ready;
                const std::string map_sig = has_map ?
                    ...
                    ( "memory|" + std::to_string( game_snap.gameplay_beatmap ? game_snap.gameplay_beatmap : game_snap.drawable_ruleset ) +
                      "|" + std::to_string( game_snap.cur_mod_state ) );
                const bool map_changed = can_try_map && map_sig != m_last_map_sig;
                const bool retry_reload = in_play && can_try_map && !beatmap_loaded;

                if ( game_snap.attached && load_state && can_try_map && ( map_changed || retry_reload ) ) {
